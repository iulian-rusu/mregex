...
namespace meta
{
     ...
     */
    template<std::size_t N, std::forward_iterator Iter, typename Storage>
    ...
    using regex_result_view = basic_regex_result<N, Iter, regex_capture_view_storage<N>>;

    /**
     * Result that holds ownership of captured content.
     */
    template<std::size_t N, std::forward_iterator Iter>
    using regex_result = basic_regex_result<N, Iter, regex_capture_storage<N>>;
    ...
    public:
        template<typename S>
        constexpr basic_regex_result(bool m, Iter offset, S &&s)
        noexcept(std::is_nothrow_move_constructible_v<Storage>)
        : matched{m}, match_begin{offset}, captures{std::forward<S>(s)}
        {}
         ...
         */
        [[nodiscard]] regex_result<N, Iter> own() const
        requires std::is_same_v<Storage, regex_capture_view_storage<N>>
        {
            ...
            return {matched, match_begin, std::move(owning_captures)};
        }
...
template<std::size_t N, std::forward_iterator Iter,  typename Storage>
std::ostream &operator<<(std::ostream &os, meta::basic_regex_result<N, Iter, Storage> const & result)
{
...
{
    template <std::size_t N, std::forward_iterator Iter,  typename Storage>
    struct tuple_size<meta::basic_regex_result<N, Iter, Storage>> : std::integral_constant<std::size_t, N> {};

    template <std::size_t ID, std::forward_iterator Iter,  std::size_t N, typename Storage>
    struct tuple_element<ID, meta::basic_regex_result<N, Iter, Storage>>
    {
        using type = decltype(std::declval<meta::basic_regex_result<N, Iter, Storage>>().template get<ID>());
    };
}
